package com.uniandes.bancandes.controllers;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;

import com.uniandes.bancandes.repository.CustomerRepository;
import com.uniandes.bancandes.repository.CustomerRepository.CustomerAggregateData;

@Controller
@RequestMapping("/customer")
public class CustomerController {

    @Autowired
    private CustomerRepository customerRepository;

    @GetMapping("/{id}")
    public String getCustomer(@PathVariable("id") int id) {
        CustomerAggregateData customer = customerRepository.getCustomerAggregateData(id);
        System.out.println(customer.getUser());
        return "customer";
    }

}
